...
if(window.XDomainRequest){
	default_params.apiroot = "http://sky-lighter.appspot.com/api/";
}
...
function ocr_region(image, col){
	...
	if(matches.length > 0){
		...
		xhr.open('GET', image.params.apiroot + 'read/' + chunk.key, true)
		xhr.send()
		xhr.onload = function(){
...
function do_lookup(image){
	...
	xhr.open('GET', image.params.apiroot + 'lookup?url=' + encodeURIComponent(image.src), true)
	xhr.send()
		...
		if(!translate.language || translate.language == 'erase'){
			...
			if(ocr.finished){
				...
				if(translate.language == 'esrever'){
					...
					var xhr = new XMLHttpRequest();
					xhr.open('POST', image.params.apiroot + 'translate', true)
...
function right_click(e){
	...
	items.push({html: "naptha <small>0.7.3</small> "+button, gray: true, action: function(){
